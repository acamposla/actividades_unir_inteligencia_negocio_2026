...
fac.Fecha_Compra, fac.Estado, fac.Comentarios 
FROM ExamenUnir.Tienda_Facturas as fac
WHERE fac.Comentarios LIKE "%contacto%"
...
Inner join ExamenUnir.Tienda_Tiendas as ten
ON emp.IDTienda = ten.IDTienda 
...
COUNT(pro.IDProducto) as Cantidad_pr,
MAX(pro.Precio_Venta) as max_price, 
AVG(pro.Precio_Venta)  as avg_price
FROM ExamenUnir.Tienda_Categoria_Productos as cat
LEFT JOIN ExamenUnir.Tienda_Productos as pro
ON cat.IDCategoria = pro.IDCategoria 
WHERE cat.Nombre_Categoria IS NOT NULL
...
SELECT ter.Comunidad, 
COUNT(fac.IDFactura ),
SUM(fac.Total_sin_impuestos) as facturacion
FROM ExamenUnir.Tienda_Territorio as ter
